...
> **Про trust-prompt (важно с Claude Code 2.1.273).** Claude при первом запуске в папке
> спрашивает «Do you trust the files in this folder?». В 2.1.273 этот диалог **перевернули**:
> по умолчанию подсвечен отказ («No, exit»). Поэтому слепой Enter, который работал раньше,
> теперь выбирает выход — сессия умирает сразу после старта, а лог при этом покажет
> «started». Скрипт лечит это двумя слоями: сначала проставляет доверие к папке в
> `~/.claude.json` (`projects."<путь>".hasTrustDialogAccepted: true`) — тогда промпт вообще
> не появляется; если он всё же вылез, скрипт жмёт **Down, затем Enter**, а не просто Enter.
>
> Если видишь «started», но бот молчит — первым делом посмотри экран сессии:
> `tmux capture-pane -t telegram -p | tail -20`. Застрявший или отклонённый диалог доверия
> виден там сразу. Порядок вариантов в диалоге Anthropic уже менял, так что не полагайся
> на «нажать N-ю клавишу» — полагайся на запись в `~/.claude.json`.
>
> **Совместимость версий.** Нажатие зависит от версии Claude Code, поэтому скрипт
> определяет её сам (`claude --version`) и выбирает поведение:
>
> | Версия Claude Code | Как принимается диалог |
> |---|---|
> | 2.1.273 и новее | `Down`, затем `Enter` (по умолчанию подсвечен отказ) |
> | старее 2.1.273 | только `Enter` (по умолчанию подсвечено согласие) |
> | определить не удалось | как для новых версий |
>
> Если у тебя версия старее 2.1.273, ничего делать не нужно — скрипт сработает по-старому.
> Обратное тоже верно: обновишь Claude Code — скрипт сам перейдёт на новое нажатие.
> Слой с записью в `~/.claude.json` от версии не зависит и работает во всех случаях.
